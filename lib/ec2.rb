...
class EC2 < Resource
  ...
  def facts
    [
      [:typed_value, instance_id = instance.instance_id, Constants.EC2Instance],
      [:typed_value, az = instance.availability_zone, Constants.Zone],
  ...
  def state_facts
    [
      [:typed_value, s = state.name, Constants.InstanceState],
      [:link, instance_id, s]
    ]
  end
